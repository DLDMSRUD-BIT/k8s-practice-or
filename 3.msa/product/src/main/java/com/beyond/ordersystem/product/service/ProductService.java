...
//        동시성 문제 해결을 위해 상품등록시 redis에 재고 세팅
    ...
    public void update(Long id, ProductUpdateDto dto){
        Product product = productRepository.findById(id).orElseThrow(()-> new IllegalArgumentException("D"));
                product.updateProduct(dto);
                if(dto.getProductImage() != null){
//                    이미지를 수정하거나 추가하거나 하는 경우
//                    기존이미지(product)를 파일명으로 삭제
...
//                    신규 이미지를 등록
                            ...
                            String newfileimgUrl = s3Client.utilities().getUrl(a->a.bucket(bucket).key(newFileName)).toExternalForm();
                            product.updateProfileImageUrl(newfileimgUrl);
                }else {
//                    이미지를 삭제하고자 하는 경우
                    if(product.getImagePath() != null) {
                        String imgUrl = product.getImagePath();
    ...
    public void updateStock(ProductStockUpdateDto dto) {
        Product product = productRepository.findById(dto.getProductId()).orElseThrow(()->new EntityNotFoundException("find by not found"));
        if(product.getStockQuantity()<dto.getProductCount()){
            throw new IllegalArgumentException("상품 제고 부족");
        }
