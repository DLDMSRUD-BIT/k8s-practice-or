...
//        opsForset(또는 Zset또는 list등 ) 존재
//        opsForsetValue는 일반 스트링 자료구조, 다양한 자료구조 존재
//        redisTemplate.opsForValue().set(member.getEmail(),  token);
        redisTemplate.opsForValue().set(member.getEmail(),  token,expiration, TimeUnit.MINUTES); //레디스 2분후 사라짐
        return token;
    }

    public Member validataRt(String refreshtoken){
        Claims claims = null;
        try {
            //        rt토큰 그 자체를 검증
...
//        redis rt와 검증
        ...
        Member member = memberRepository.findByEmail(email).orElseThrow(()->new EntityNotFoundException("error"));
//        redis rt와 비교 검증
